...
import SearchUsersInputModel from '../../domain/user/input-model/search_users.input_model';
import SearchUsersOutputModel from '../../domain/user/use-case/output-model/search_users.output_model';
import { SearchedUserDTO } from '../../domain/user/use-case/persistence-dto/searched_user.dto';
...
export class SearchUsersService implements SearchUsersInteractor {
  ...
  async execute(input: SearchUsersInputModel): Promise<SearchUsersOutputModel> {
    ...
    const users: SearchedUserDTO[] = await this.gateway
      .findAll({ email, name })
      .then((result: UserDTO[]) => {
